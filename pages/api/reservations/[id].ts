...
export default async function handler(
  ...
  const reservationId = parseInt(
    typeof req.query.id === "string" ? req.query.id : "-1"
  );
  ...
  if (reservation === null) {
    return res
      .status(200)
      .json({ message: "no reservation with such id", data: reservation });
  }
  ...
  if (req.method === "GET") {
    ...
    return res
      .status(200)
      .json({ message: "updated success", data: updatedReservation });
